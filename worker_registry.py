...
class WorkerInfo:
    ...
    worker_id: str          # e.g. "musicgen-1", "ace_step_v15-1"
    model: str              # "musicgen"
    port: int               # 9102
...
class WorkerRegistry:
    ...
    def __init__(self, port_min: int = 9151, port_max: int = 9249):
    ...
    def allocate_port(self) -> int:
        ...
        with self._lock:
            if not self._port_pool:
                raise RuntimeError("No available ports in pool (all ports in use)")
    ...
    def next_worker_id(self, model: str) -> str:
        """Generate the next worker ID for a model (e.g. musicgen-1, musicgen-2)."""
        with self._lock:
    ...
    def pick_worker(self, model: str) -> WorkerInfo | None:
        """Round-robin among ready workers for a given model."""
        ready = self.get_ready_workers(model)
        if not ready:
            return None
        with self._lock:
    ...
    def pick_and_mark_busy(self, model: str, job_id: str) -> WorkerInfo | None:
        """Atomically pick a ready worker and mark it busy. Prevents TOCTOU races."""
        with self._lock:
    ...
    def mark_ready(self, worker_id: str) -> None:
        """Mark a worker as ready for new work. Only transitions from 'busy' status
        to prevent resurrecting dead/removed workers."""
        with self._lock:
            w = self._workers.get(worker_id)
