...
from config import (
    BASE_DIR, PYTHON_PATH, VENVS_DIR, WORKER_PORT_MIN, WORKER_PORT_MAX,
...
# Model -> venv mapping
MODEL_VENV_MAP = {
    "ace_step_v15": "ace_step_env",
    "ace_step_v1": "ace_step_v1_env",
    "heartmula": "heartmula_env",
    "diffrhythm": "diffrhythm_env",
    "yue": "yue_env",
    "musicgen": "musicgen_env",
    "riffusion": "riffusion_env",
    "stable_audio": "stable_audio_env",
}


class WorkerManager:
    """Manages the lifecycle of music worker subprocesses."""
    ...
    async def spawn_worker(self, model: str, device: str | None = None) -> WorkerInfo:
        ...
        Args:
            model: Model identifier (musicgen, ace_step_v15, etc.)
        ...
        worker_script = str(BASE_DIR / "music_worker.py")

        # Use per-model venv Python if available, fall back to embedded Python
        venv_name = MODEL_VENV_MAP.get(model)
        venv_python = VENVS_DIR / venv_name / "Scripts" / "python.exe" if venv_name else None
        if venv_python and venv_python.exists():
            python_exe = str(venv_python)
            logger.info("Using venv Python for %s: %s", model, python_exe)
        else:
            python_exe = str(PYTHON_PATH)
            if venv_name:
                logger.warning(
                    "Venv Python not found for %s at %s, falling back to embedded Python",
                    model, venv_python,
                )
        ...
        # Redirect download cache but keep token at ~/.cache/huggingface
        env.setdefault("HF_HUB_CACHE", str(MODELS_DIR / "hub"))
    ...
    async def _wait_for_healthy(self, worker: WorkerInfo) -> None:
        """Poll worker /health until FastAPI responds."""
    ...
    async def kill_worker(self, worker_id: str) -> bool:
        """Gracefully stop and remove a worker. Returns True if found and killed."""
